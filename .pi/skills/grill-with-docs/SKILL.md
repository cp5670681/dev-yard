...
- **死守总轮次（Max 3 轮，是上限不是配额）**：小需求 1 轮，中需求 1~2 轮，大需求 2~3 轮。不要为了凑满一轮而提问；但 0 题必须由下面的「Frontier 清点」挣得，不能因为「看起来清楚」就默认收敛。
- **明知故问即缺陷**：答案若已在 `REQUIREMENT.md`、验收标准、`GRILL.md`「关键事实（已自查）」或源码里，就**不是问题**，落进「默认假设与实现细节」。推荐项若与需求/验收标准冲突，整题作废。
- **外部 / 跨团队契约与产品意图必须问**：新增 AI function、请求/响应 schema、callback 路径、服务归属，以及需求沉默或含糊的产品规则（覆盖 vs 合并、触发口径、时间窗口）、范围边界——即便有相似功能可抄，也是问题不是默认。抄一个相似功能只决定「怎么实现」，不决定「新契约是什么」。
- **按规模弹性展开 + 模块分组**：多题时（≥ 4）必须按模块/领域分组并给出推荐选项（`➡️`），方便批量审批。
- **合理默认假设（Sensible Defaults）**：仅限次要**实现**细节与非关键边缘分支（表/列、同步范围、角色、页面入口等一律自查后默认），记录在 `GRILL.md` 的「默认假设与实现细节」中。落默认前先对照 `REQUIREMENT.md` 与验收标准：**与需求冲突的默认不是默认，要作为问题提出**。
- **收敛与停机**：主链路闭环、无阻塞性分歧，**且已在 `GRILL.md` 写下 `### Frontier 清点`**（逐个设计树分支标 `已问`/`默认`/`不适用`）后立即停止；0 题合法**但必须是被清点证明为空的 frontier**（web 轮次写 `done: true` 时同样要留下清点）。提示用户进入 `dev-yard spec <JIRA>`。不要自己写 spec、不要 freeze、不要写实现。
