...
- **死守总轮次（Max 3 轮，是上限不是配额）**：小需求 1 轮，中需求 1~2 轮，大需求 2~3 轮。不要为了凑满一轮而提问。
...
- **合理默认假设（Sensible Defaults）**：次要实现细节与非关键边缘分支直接采用业内最佳实践（表/列、同步范围、角色、页面入口等一律自查后默认），记录在 `GRILL.md` 的「默认假设与实现细节」中，无需占用提问轮次。
- **收敛与停机**：主链路闭环且无阻塞性分歧时立即停止；**0 题也是合法结果**（web 轮次直接写 `done: true`），提示用户进入 `dev-yard spec <JIRA>`。不要自己写 spec、不要 freeze、不要写实现。
