...
| `dev-yard req test` | qa-design + qa-run（提测后设计用例，**暂停等人工审核**；`--approve` 通过后执行，`--redesign` 配合 `--feedback`/`--feedback-file` 带意见重做；设计期宿主跑 `data.verify` 核实数据前置，失败自动回灌重做，`--verify-only` 只核实、`--no-verify` 跳过、`--allow-unverified` 越权放行；产物在 `reqs/<JIRA>/qa/`） |
